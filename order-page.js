(()=>{
  ...
  function render(){
    ...
    document.querySelector('#addBuild').onclick=()=>{
      ...
      WD.addCart({id:p.id,name:p.name,price:calc(),basePrice:basePrice(),detail,sizeName:size[0],extraNames:extras.map(x=>x[0]),itemNote:'',image:imgFor(sizeIndex),promoType:promo?.type||'',promoTitle:promo?.title||'',freebie:promo?.type==='free_item'?promo.freeItem||'Free Coke':''});
