(() => {
  ...
  function getEightPeople() {
    ...
    const savedByName = new Map(saved.map(person => [String(person.name || '').toLowerCase(), person]));

    return order.map(id => {
      const fallback = defaults.find(person => String(person.id) === id) || {};
      const override = savedById.get(id) || savedByName.get(String(fallback.name || '').toLowerCase()) || {};
      return {...fallback, ...override, id};
    ...
    return `
      <section class="team-founder-v3" aria-labelledby="founderName">
        <div class="team-founder-copy-v3">
          <div class="team-founder-number">01 / 08</div>
  ...
  function openProfile(person, index) {
    ...
    content.innerHTML = `
      <div class="team-drawer-portrait-v3">
        ...
        <span class="team-drawer-count-v3">${number} / 08</span>
