(() => {
  ...
  function getTeamPeople() {
    ...
    return order.map(id => {
      ...
      // Names and public card presentation are intentionally locked to the
      // current Wrap District team. Saved bios, socials and non-stock image
      // uploads can still update the profile drawer.
      return {
        ...fallback,
        bio: override.bio || fallback.bio,
        instagram: override.instagram ?? fallback.instagram,
        tiktok: override.tiktok ?? fallback.tiktok,
        snapchat: override.snapchat ?? fallback.snapchat,
        x: override.x ?? fallback.x,
        facebook: override.facebook ?? fallback.facebook,
        image: isAllowedTeamImage(override.image, fallback.image),
        id,
        name: fallback.name
      };
    ...
    return `
      <section class="team-founder-v3" aria-labelledby="founderName">
        <div class="team-founder-copy-v3">
          ...
          <h2 id="founderName">Kenneth<br>Kankam</h2>
          <p>Wrap District began with Kenneth and his love for making food people genuinely want to come back for. He founded the District and still leads the cooking, menu ideas and food standards behind it.</p>
          <button class="team-profile-link-v3" type="button" data-person-id="${esc(person.id)}">
            Open Kenneth's profile <span><i class="fa-solid fa-arrow-right"></i></span>
          </button>
        </div>
        <button class="team-founder-portrait-v3" type="button" data-person-id="${esc(person.id)}" aria-label="Open Kenneth Kankam's profile">
          <img src="${esc(person.image)}" alt="Kenneth Kankam">
