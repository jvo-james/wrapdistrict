...
The security rules and District Control access use the existence of `/admins/{uid}`. The admin can later change the Firebase Auth email from **Settings → Account & security** without changing the UID or losing admin access. `adminEmails` remains only a fallback for offline/local development.
...
Paystack's browser `onSuccess` callback drives the user experience, while authoritative verification is now handled by Netlify Functions. `netlify/functions/verify-payment.js` verifies the transaction directly with Paystack and `netlify/functions/paystack-webhook.js` receives signed `charge.success` events. Never put the Paystack secret key in `config.js` or any frontend file.

Before opening Paystack, checkout creates a **Pending** Firestore order and continuously saves a recoverable `abandonedCarts` record. The webhook can therefore complete the same order even if the customer closes the browser after payment.
