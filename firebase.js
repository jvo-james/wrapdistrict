(() => {
  ...
  async function isAdmin(user = auth?.currentUser) {
    ...
    if (!(await init())) {
      const emails=(WD_CONFIG.adminEmails||[]).map(x=>String(x).toLowerCase());
  ...
  async function reauthenticate(password){
    await init();
    const user=auth.currentUser;if(!user)throw new Error('You are not signed in.');
    const credential=firebase.auth.EmailAuthProvider.credential(user.email,password);
    await user.reauthenticateWithCredential(credential);return user;
  }
  async function changePassword(currentPassword,newPassword){const user=await reauthenticate(currentPassword);await user.updatePassword(newPassword);return true;}
  async function changeEmail(currentPassword,newEmail){const user=await reauthenticate(currentPassword);if(typeof user.verifyBeforeUpdateEmail==='function')await user.verifyBeforeUpdateEmail(newEmail);else await user.updateEmail(newEmail);return true;}
  async function adminStatus() {
    const ok=await init();
    if(!ok||!auth)return {user:null,admin:false};
    return new Promise(resolve => auth.onAuthStateChanged(async user => resolve({ user, admin: user ? await isAdmin(user) : false })));
  }

  window.WD_DB = { init, list, save, remove, create, getSettings, saveSettings, signIn, signOut, currentUser, isAdmin, adminStatus, changePassword, changeEmail, cache };
})();
